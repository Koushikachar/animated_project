    ...
    >
      ...
      <div className="hidden md:flex">
        ...
        >
          ...
          <div
            className="absolute translate-y-12 transition-all duration-300
          group-hover:translate-y-0 font-bold text-[#ffd900]"
          >
            Let's Talk
          </div>
        </Link>
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-[#ffd900] border-t border-black flex flex-col items-center py-4 gap-4 md:hidden  ">
          {["Work", "About", "MainService", "Contact"].map((item) => (
            <Link
              key={item}
              to={`${item}`}
              onClick={() => setMenuOpen(false)}
              className="w-11/12 text-center bg-black text-white py-3 uppercase border rounded-[5px]"
            >
              {item === "MainService" ? "Services" : item}
            </Link>
          ))}
