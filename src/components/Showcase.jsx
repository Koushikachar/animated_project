...
const LogoSlot = ({ logo1, logo2, index }) => {
  ...
  return (
    <div className="relative h-14 sm:h-16 lg:h-20 w-full justify-center items-center">
      <img
        src={logo1}
        alt="partner logo1"
        className={`absolute h-full object-contain transition-all duration-1000 ease-in-out ${showFirst ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
      />
      <img
        src={logo2}
        alt="partner logo2"
        className={`absolute h-full object-contain transition-all duration-1000 ease-in-out ${showFirst ? "opacity-0 scale-95" : "opacity-100 scale-100"}`}
      />
    </div>
  );
};
...
const Showcase = () => {
  ...
  return (
    ...
    >
      <div className="w-ful overflow-hidden rounded-lg">
        <img
          data-scroll
